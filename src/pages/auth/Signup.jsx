...
const Signup = ({setCurrentPage}) => {
  ...
  const handleSignup = async(e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(API_PATHS.AUTH.REGISTER, {name: fullName, password, email, profileImageUrl});
    ...
    <div className='w-[90vw] md:w-[33vw] p-7 flex flex-col justify-center'>
      ...
      <form onSubmit={handleSignup} action="">
        ...
        <div className='grid grid-cols-1 md:grid-cols-1 gap-2'>
          ...
          <p className='text-[13px] text-slate-800 mt-3'>Already have an account ? {" "} <button className='text-primary font-medium underline cursor-pointer' onClick={() => setCurrentPage('login')}>Login</button></p>
        </div>
      </form>
    </div>
  )
