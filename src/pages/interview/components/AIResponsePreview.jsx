      ...
      <div className='text-[14px] prose prose-slate dark:prose-invert max-w-none'>
        ...
        components={{
          code({node, className, children, ...props}){
            ...
            return !isInline ? (<CodeBlock code={String(children).replace(/\n$/, '')} language={language} />) : (<code className='px-1 py-0.5 bg-gray-100 rounded text-sm' {...props}>{children}</code>)
          },
          p({children}){
...
function CodeBlock({code, language}){
  ...
  return(
    <div className='relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200'>
      ...
      <SyntaxHighlighter language={language} style={oneLight} customStyle={{fontSize: 12.5, margin: 0, padding: '1rem', background: 'transparent'}}>{code}</SyntaxHighlighter>
    </div>
  );
}
