...
//                           <button className='flex items-center gap-3 text-sm text-white font-medium bg-black px-5 py-2 mr-2 rounded text-nowrap cursor-pointer ' onClick={uploadMoreQuestions} disabled={isLoading || isUpdateLoader}>
...
import SpinnerLoader from '../../components/loaders/SpinnerLoader';
import { AnimatePresence, motion } from "framer-motion";
import { LuCircleAlert, LuListCollapse, LuListChecks } from 'react-icons/lu';
import toast from 'react-hot-toast';
import RoleInfoHeader from './components/RoleInfoHeader';
import axiosInstance from '../../utils/axiosInstance';
import { API_PATHS } from '../../utils/apiPaths';
...
const InterviewPrep = () => {
  ...
  const toggleQuestionPinStatus = async (questionId) => {
    try {
      const response = await axiosInstance.post(API_PATHS.QUESTION.PIN(questionId));
      if (response.data && response.data.question) fetchSessionDetailsById();
    } catch (error) {
      console.error("Error : ", error);
    }
  };
              ...
              {questions.map((data, index) => (
                ...
                >
                  <>
                    ...
                    {/* Load More + Take Quiz — shown after last question */}
                    {!isLoading && questions.length === index + 1 && (
                      <div className='flex items-center justify-center gap-3 mt-5'>
                        ...
                        <button
                          className='flex items-center gap-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 px-5 py-2 rounded text-nowrap cursor-pointer transition-colors'
